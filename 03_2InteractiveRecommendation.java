...
public class InteractiveRecommendation {
	...
	private int generateNextQuestion_IHS_MMAS() {
		...
		IHS_MMAS = new InteractiveHeuristicSearch_MMAS(itemToTagMatrix,
                                                       weightOfItem,
                                                       tagHasNotBeenAsked);
		idxOfSelTag = IHS_MMAS.generateNextQuestion();
	...
	private void readDataMatrixFromFile() throws IOException {
		...
		numOfItem = Integer.parseInt(inputFile.readLine());
		numOfTag = Integer.parseInt(inputFile.readLine());
		numOfOne = Integer.parseInt(inputFile.readLine());
		inputFile.readLine();
		...
		for(int i = 0; i < numOfItem; i++) {
			itemName[i] = inputFile.readLine();
		}
		inputFile.readLine();
		
		for(int j = 0; j < numOfTag; j++) {
			tagName[j] = inputFile.readLine();
		}
		inputFile.readLine();
